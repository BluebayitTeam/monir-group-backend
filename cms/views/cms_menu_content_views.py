...
def updateCMSMenuContent(request, pk):
	...
	try:
		menu_content = CMSMenuContent.objects.get(pk=pk)
		print('menu content.name: ', menu_content.name)
		content_images = CMSMenuContentImage.objects.filter(head=menu_content.name)
		print('content images: ', content_images)
	except ObjectDoesNotExist:
		return Response({'detail': f"CMSMenuContent id - {pk} doesn't exists"})

	serializer = CMSMenuContentSerializer(menu_content, data=data)
	if serializer.is_valid():
		serializer.save()
		content_images.update(head=name)
