...
# @permission_classes([IsAuthenticated])
# @has_permissions([PermissionEnum.ATTRIBUTE_LIST.name])
def getAllHomePageSliderWP(request):
	homepage_sliders = HomePageSlider.objects.all().order_by("serial_number")
...
def getFirstHomePageSlider(request):
    homepage_slider = HomePageSlider.objects.first()

    if not homepage_slider:
        return JsonResponse({'detail': 'No slider found.'}, status=404)

    serializer = HomePageSliderListSerializer(homepage_slider)
    response = {
        'homepage_slider': serializer.data,
    }
